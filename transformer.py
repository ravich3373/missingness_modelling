...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=48, nhead=2, num_encoder_layers=2,
        ...
        self.encoder = TransformerEncoder(encoder_layer, num_encoder_layers, d_model, encoder_norm)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src,
        ...
        batch_size, feat_count, _ = output.shape
        output = self._mean_logvar_readout(output)  # shape  (batch_size, input_dim, 2 * transformer_embedding_dim)
        mean, logvar = output.chunk(chunks=2, dim=-1)

        mean = mean.reshape(batch_size, feat_count*self.d_model)
        logvar = logvar.reshape(batch_size, feat_count*self.d_model)
        
        # Clamp encoder_logvar (better numerical stability)
        encoder_logvar = torch.clamp(logvar, -20, 20)
        encoder_stddev = torch.sqrt(torch.exp(encoder_logvar))

        gaussian = tdist.Normal(mean, encoder_stddev)
        samples = gaussian.rsample((sample_count,)).to(mean.device)  # Shape (count, batch_size, latent_dim)

        samples = samples.reshape(batch_size, feat_count, self.d_model)
        return samples, (mean, encoder_logvar)
        # return output


class TransformerDecoder(nn.Module):

    def __init__(self, decoder_layer, num_layers, d_model, norm=None, return_intermediate=False):
        super().__init__()
