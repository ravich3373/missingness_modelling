...
def negative_log_likelihood(
    ...
    if sum_type is None:
        nlls = torch.zeros(batch_size, t_cont.shape[1]+t_cat.shape[1])
    else:
        nlls = torch.zeros(1, t_cont.shape[1]+t_cat.shape[1]) # num vars replace by the sum
    ...
    cat_idx_nlls = alpha * categorical_negative_log_likelihood(
        t_cat,
        cat_mean,
        m_cat,
        sum_type=feature_sum_type,
